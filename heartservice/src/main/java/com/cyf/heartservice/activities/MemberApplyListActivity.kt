...
import com.cyf.heartservice.entity.DJSQJL
import com.cyf.heartservice.entity.MemberApplyInfo
import com.cyf.heartservice.entity.MemberApplyInfoListRes
import com.cyf.heartservice.entity.getInterface
import com.dou361.dialogui.listener.DialogUIListener
import com.google.gson.Gson
import kotlinx.android.synthetic.main.activity_member_apply_list.*
...
class MemberApplyListActivity : MyBaseActivity() {

    private val data = ArrayList<MemberApplyInfo>()
    private val adapter = MemberApplyListAdapter(data)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentBaseView(R.layout.activity_member_apply_list)
        setTitle("党员认证")
        init()
    }
    ...
    private fun init() {
        ...
        adapter.myOnItemClickListener = object : MyBaseAdapter.MyOnItemClickListener {
            override fun onItemClick(parent: MyBaseAdapter, view: View, position: Int) {
                val intent = Intent(view.context, MemberApplyDetailsActivity::class.java)
    ...
    private fun getMemberApplyList(page: Int, isRefresh: Boolean = false) {
        ...
        KevinRequest.build(this).apply {
            setRequestUrl(DJSQJL.getInterface())
            ...
            setSuccessCallback(object : KevinRequest.SuccessCallback {
                override fun onSuccess(context: Context, result: String) {
                    val memberApplyInfoListRes =
                        Gson().fromJson(result, MemberApplyInfoListRes::class.java)
                    listViewSwipe.setTotalPages(memberApplyInfoListRes.retCounts, 15)
                    if (isRefresh) {
                        data.clear()
                    }
                    data.addAll(memberApplyInfoListRes.retRes)
    ...
    private class MemberApplyListAdapter(val data: ArrayList<MemberApplyInfo>) :
        MyBaseAdapter(R.layout.layout_member_apply_list_item) {

        override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
            super.onBindViewHolder(holder, position)
            val memberApplyInfo = data[position]
            holder.itemView.name.text = memberApplyInfo.title
            holder.itemView.status.text = when (memberApplyInfo.sh_status) {
                1 -> {
            ...
            holder.itemView.time.text = CalendarUtil(memberApplyInfo.create_time, true).format(
