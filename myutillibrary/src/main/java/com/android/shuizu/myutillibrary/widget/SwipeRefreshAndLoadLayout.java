...
public class SwipeRefreshAndLoadLayout extends ViewGroup implements NestedScrollingParent,
    ...
    private static final String LOG_TAG = SwipeRefreshAndLoadLayout.class.getSimpleName();
    ...
    boolean mNotify;
    // Whether the client has set_icon a custom starting position;
                    ...
                    if (mListener != null) {
                        if (isLoadMore) {
                            if (totalPages > 0 && currPage >= totalPages) {
                                ...
                                currPage++;
                                mListener.onLoadMore(currPage, totalPages);
                            }
                        } else {
                            currPage = 1;
                            mListener.onRefresh();
     ...
     *              spinner is set_icon to appear. Setting it to true will cause indicator to be scaled
    ...
    public boolean canChildScrollUp() {
        ...
        if (mChildScrollUpCallback != null) {
            rel = mChildScrollUpCallback.canChildScrollUp(this, mTarget);
        } else {
            rel = ViewCompat.canScrollVertically(mTarget, -1);
        }
        isLoadMore = rel;
        return rel;
    }
    ...
    public boolean canChildScrollDown() {
        boolean rel;
        if (mChildScrollUpCallback != null) {
            rel = mChildScrollUpCallback.canChildScrollDown(this, mTarget);
        } else {
            rel = ViewCompat.canScrollVertically(mTarget, 1);
        }
        isLoadMore = !rel;
    ...
    /**
     * Set a callback to override {@link SwipeRefreshAndLoadLayout#canChildScrollUp()} method. Non-null
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        if (dy < 0 && mTotalUnconsumed > 0) {
            if (Math.abs(dy) > mTotalUnconsumed) {
                consumed[1] = Math.abs(dy) - (int) mTotalUnconsumed;
                mTotalUnconsumed = 0;
            } else {
        ...
        if (mUsingCustomStart && dy != 0 && mTotalUnconsumed == 0
                && Math.abs(Math.abs(dy) - consumed[1]) > 0) {
            mCircleView.setVisibility(View.GONE);
        }

        // Now let our nested parent consume the leftovers
        final int[] parentConsumed = mParentScrollConsumed;
        if (dispatchNestedPreScroll(dx - consumed[0], Math.abs(dy) - consumed[1], parentConsumed, null)) {
    ...
    public interface OnRefreshListener {
        ...
        void onLoadMore(int currPage, int totalPages);
    }

    /**
     * Classes that wish to override {@link SwipeRefreshAndLoadLayout#canChildScrollUp()} method
     * behavior should implement this interface.
     */
    public interface OnChildScrollUpCallback {
        /**
         * Callback that will be called when {@link SwipeRefreshAndLoadLayout#canChildScrollUp()} method
         ...
         */
        boolean canChildScrollUp(SwipeRefreshAndLoadLayout parent, @Nullable View child);

        boolean canChildScrollDown(SwipeRefreshAndLoadLayout parent, @Nullable View child);
    }
}
