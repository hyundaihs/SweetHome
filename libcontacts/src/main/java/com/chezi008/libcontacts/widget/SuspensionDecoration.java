 ...
 */
 public class SuspensionDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int COLOR_TITLE_BG = Color.parseColor("#FFFFFFFF");
    private static int COLOR_TITLE_FONT = Color.parseColor("#FFA0A0A0");
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {
                    ...
                    if (!TextUtils.isEmpty(mDatas.get(position).getSuspensionTag()) && !mDatas.get(position).getSuspensionTag().equals(mDatas.get(position - 1).getSuspensionTag())) {
