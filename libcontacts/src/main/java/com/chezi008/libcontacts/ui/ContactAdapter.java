...
public class ContactAdapter extends RecyclerView.Adapter<ContactAdapter.ContactAdapterHolder> {
    ...
    public int getLetterPosition(String letter) {
        for (int i = 0; i < mData.size(); i++) {
            if (mData.get(i).getPy().equals(letter)) {
                return i;
            }
        }
        return -1;
    ...
    public void onBindViewHolder(@NonNull final ContactAdapterHolder contactAdapterHolder, int position) {
        ...
        contactAdapterHolder.tvName.setText(contactBean.getTitle());
        contactAdapterHolder.tvContent.setText(contactBean.getType_title()+"  "+contactBean.getXq_title());
        ...
        if (contactBean.getAvatar() != null && contactBean.getAvatar().contains("intres")) {
            ...
            contactListener.loadAvatar(contactAdapterHolder.ivAvatar, contactBean);
        }
        //checkbox
    ...
    static class ContactAdapterHolder extends RecyclerView.ViewHolder {
        TextView tvName,tvNum,tvContent;
        ImageView ivAvatar;
        CheckBox cbCheck;
